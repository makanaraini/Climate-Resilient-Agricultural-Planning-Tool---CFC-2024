...
@app.route('/api/farm-dashboard', methods=['GET'])
@jwt_required()
def get_farm_dashboard():
    current_user = get_jwt_identity()
    print(f"Fetching dashboard for user: {current_user}")
    if not current_user:
        return jsonify({"error": "You must be logged in to view this dashboard."}), 401
    ...
    dashboard_data = {
        ...
        "cropData": [
            {"crop": "Wheat", "yield": 30, "area": 20},
            {"crop": "Corn", "yield": 25, "area": 15},
            {"crop": "Soybeans", "yield": 15, "area": 15}
        ]
    }
...
def get_user():
    current_user = get_jwt_identity()
    print(f"Fetching user data for: {current_user}")
    user_data = users.get(current_user, {})
